...
def map_to_ocsf(
    ...
    class_uid = parser.get("ocsf_class_uid", 1001)
    ...
    _apply_provenance(ev, parser, class_uid, drain3_template is not None)
...
def _apply_provenance(ev: dict, parser: dict, class_uid: int, is_drain3: bool):
    ...
    evaluator exactly which pipeline stage we are sure of.
    """
    ...
    if parser.get("parser_id") == "DLQ":
        ...
        ev["ocsf_mapping_status"] = "unmapped"
        ev["needs_review"] = True
        ev["confidence_breakdown"] = {k: 0.0 for k in ev["confidence_breakdown"]}
        return
