...
# "serviceName: com.foo.Bar" / "serviceName=com.foo.Bar" — the service identity
# is confidently present when logged; promote it as a semantic field (spec #4).
_SERVICE_NAME_RE = re.compile(r"\bservice[_ ]?name[:=]\s*(?P<service_name>\S+)",
                              re.IGNORECASE)
...
def classify(ev: AndroidEvent) -> SemanticResult:
    res = _classify_core(ev)
    # #4: attach the confidently-extractable service identity (if present) to
    # whatever class the core taxonomy landed on, without altering the class.
    # `service.name` has no slot in the uniform envelope skeleton, so the value
    # is preserved losslessly under `unmapped` (schema stays identical across
    # families) rather than promoted to a new top-level OCSF key.
    m = _SERVICE_NAME_RE.search(ev.message or "")
    if m and m["service_name"]:
        res.fields["service_name"] = m["service_name"]
    return res


def _classify_core(ev: AndroidEvent) -> SemanticResult:
    tag = (ev.tag or "").strip()
    tag_low = tag.lower()
